$(function() {
    function t(t, e, a, n, r, i, s, l) {
        function o() {
            A.attr("transform", null).transition().duration(i).ease("linear").attr("transform", "translate(" + m(0) + ",0)"),
            "area" == e ? A.attr("d", v).attr("class", "d3-area").style("fill", l) : A.attr("d", y).attr("class", "d3-line d3-line-medium").style("stroke", l)
        }
        function d() {
            p = c.node().getBoundingClientRect().width - u.left - u.right,
            x.attr("width", p + u.left + u.right),
            b.attr("width", p + u.left + u.right),
            m.range([0, p]),
            k.attr("width", p),
            b.select(".d3-line").attr("d", y),
            b.select(".d3-area").attr("d", v)
        }
        for (var c = d3.select(t), u = {
            ...
            bottom: 0,
            left: 0
        }, p = c.node().getBoundingClientRect().width - u.left - u.right, n = n - u.top - u.bottom, f = [], h = 0; h < a; h++)
        ...
        "area" == e ? A.attr("d", v).attr("class", "d3-area").style("fill", l) : A.attr("d", y).attr("class", "d3-line d3-line-medium").style("stroke", l),
        A.style("opacity", 0).transition().duration(750).style("opacity", 1),
        ...
        $(window).on("resize", d),
        $(document).on("click", ".sidebar-control", d)
    }
    function e(t, e, a, n) {
        function r() {
            var t = d3.interpolate(0, o);
            return function(e) {
                var a = o / (100 / e)
                  , n = d.endAngle(l * a);
                return d(t(n))
        ...
        var i = d3.select(t)
          ...
          , l = 2 * Math.PI
          , o = $(t).data("progress")
          , d = d3.svg.arc().startAngle(0).innerRadius(0).outerRadius(s).endAngle(function(t) {
        ...
        p.append("path").attr("d", d.endAngle(l)).style("fill", "#fff").style("stroke", n).style("stroke-width", 1.5),
        p.append("path").style("fill", n).transition().ease("cubic-out").duration(2500).attrTween("d", r)
    }
    function a(t, e, a, n, r, i, s, l, o) {
        function d() {
            ...
            y.attr("width", f),
            h.rangeBands([0, f], .3),
            y.selectAll(".d3-random-bars").attr("width", h.rangeBand()).attr("x", function(t, e) {
                return h(e)
            })
        }
        for (var c = [], u = 0; u < e; u++)
            c.push(Math.round(10 * Math.random()) + 10);
        var p = d3.select(t)
          , f = p.node().getBoundingClientRect().width
          , h = d3.scale.ordinal().rangeBands([0, f], .3)
          , m = d3.scale.linear().range([0, a]);
        h.domain(d3.range(0, c.length)),
        m.domain([0, d3.max(c)]);
        var g = p.append("svg")
          , y = g.attr("width", f).attr("height", a).append("g")
          , v = y.selectAll("rect").data(c).enter().append("rect").attr("class", "d3-random-bars").attr("width", h.rangeBand()).attr("x", function(t, e) {
            return h(e)
        }).style("fill", l)
          , x = d3.tip().attr("class", "d3-tip").offset([-10, 0]);
        "hours" != o && "goal" != o && "members" != o || v.call(x).on("mouseover", x.show).on("mouseout", x.hide),
        "hours" == o && x.html(function(t, e) {
        ...
        n ? function() {
            v.attr("height", 0).attr("y", a).transition().attr("height", function(t) {
                return m(t)
            }).attr("y", function(t) {
                return a - m(t)
            }).delay(function(t, e) {
    ...
    function n(t, e, a, n, r, i, s, l) {
        function o(t) {
            x.attr("d", v.endAngle(u * t)),
            b.attr("d", v.endAngle(u * t)),
            w.text(p(t))
        }
        var d = d3.select(t)
          , c = r
        ...
        y.append("path").attr("class", "d3-progress-background").attr("d", v.endAngle(u)).style("fill", "#eee");
        var x = y.append("path").attr("class", "d3-progress-foreground").attr("filter", "url(#blur)").style("fill", n).style("stroke", n)
        ...
        d3.select(t).append("i").attr("class", i + " counter-icon").attr("style", "top: " + (f - 32) / 2 + "px"),
        d3.select(t).append("div").text(s),
        d3.select(t).append("div").attr("class", "text-size-small text-muted").text(l);
        var k = 0;
        !function t() {
            o(k),
            h > 0 && (h--,
    ...
    Array.prototype.slice.call(document.querySelectorAll(".switch")).forEach(function(t) {
        new Switchery(t,{
            color: "#4CAF50"
        })
    }),
    ...
    function(t, e) {
        function a() {
            ...
            y.attr("width", i + r.left + r.right),
            v.attr("width", i + r.left + r.right),
            c.range([0, i]),
            v.selectAll(".d3-axis-horizontal").call(f),
            v.selectAll(".d3-axis-subticks").attr("x1", c).attr("x2", c),
            v.selectAll(".d3-grid-dashed").call(g.tickSize(-i, 0, 0)),
            v.selectAll(".d3-axis-right").attr("transform", "translate(" + i + ", 0)"),
            v.selectAll(".streamgraph-layer").attr("d", function(t) {
                return w(t.values)
            })
        }
        var n = d3.select(t)
          ...
          , h = d3.svg.axis().scale(u).ticks(6).innerTickSize(4).outerTickSize(0).tickPadding(8).tickFormat(function(t) {
            return t / 1e3 + "k"
        })
          , m = h
          ...
          , x = d3.layout.stack().offset("silhouette").values(function(t) {
            return t.values
        }).x(function(t) {
            return t.date
        }).y(function(t) {
          ...
          , b = d3.nest().key(function(t) {
            return t.key
        })
          , w = d3.svg.area().interpolate("cardinal").x(function(t) {
            return c(t.date)
        }).y0(function(t) {
            return u(t.y0)
        }).y1(function(t) {
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function(a, d) {
            d.forEach(function(t) {
                t.date = l.parse(t.date),
                t.value = +t.value
            });
            var y = x(b.entries(d));
            c.domain(d3.extent(d, function(t, e) {
                return t.date
            })),
            u.domain([0, d3.max(d, function(t) {
                return t.y0 + t.y
            })]),
            v.append("g").attr("class", "d3-grid-dashed").call(g);
            var k = v.append("g").attr("class", "streamgraph-layers-group")
              , A = k.selectAll(".streamgraph-layer").data(y).enter().append("path").attr("class", "streamgraph-layer").attr("d", function(t) {
                return w(t.values)
            }).style("stroke", "#fff").style("stroke-width", .5).style("fill", function(t, e) {
                return p(e)
            })
              , z = A.style("opacity", 0).transition().duration(750).delay(function(t, e) {
                return 50 * e
            }).style("opacity", 1);
            v.append("g").attr("class", "d3-axis d3-axis-left d3-axis-solid").call(h.orient("left")),
            d3.select(v.selectAll(".d3-axis-left .tick text")[0][0]).style("visibility", "hidden"),
            v.append("g").attr("class", "d3-axis d3-axis-right d3-axis-solid").attr("transform", "translate(" + i + ", 0)").call(m.orient("right")),
            d3.select(v.selectAll(".d3-axis-right .tick text")[0][0]).style("visibility", "hidden"),
            v.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")").call(f).selectAll(".d3-axis-subticks").data(c.ticks(d3.time.hours), function(t) {
                return t
            }).enter().append("line").attr("class", "d3-axis-subticks").attr("y1", 0).attr("y2", 4).attr("x1", c).attr("x2", c);
            var C = k.append("g").attr("class", "hover-line")
            ...
            z.each("end", function() {
                A.on("mouseover", function(t, e) {
                    v.selectAll(".streamgraph-layer").transition().duration(250).style("opacity", function(t, a) {
                        return a != e ? .75 : 1
                    })
                    ...
                    mousey = mouse[1],
                    datearray = [];
                    var a = c.invert(mousex);
                    a = a.getHours();
                    for (var n = t.values, r = 0; r < n.length; r++)
                        datearray[r] = n[r].date,
                        datearray[r] = datearray[r].getHours();
                    mousedate = datearray.indexOf(a),
                    pro = t.values[mousedate].value,
                    B.attr("x", mousex - 3).attr("y", mousey - 6).style("opacity", 1),
                    M.attr("x1", mousex).attr("x2", mousex).style("opacity", 1),
                    s.html("<ul class='list-unstyled mb-5'><li><div class='text-size-base mt-5 mb-5'><i class='icon-circle-left2 position-left'></i>" + t.key + "</div></li><li>Visits: &nbsp;<span class='text-semibold pull-right'>" + pro + "</span></li><li>Time: &nbsp; <span class='text-semibold pull-right'>" + o(t.values[mousedate].date) + "</span></li></ul>").style("display", "block"),
                    s.append("div").attr("class", "d3-tip-arrow")
                }).on("mouseout", function(t, e) {
                    v.selectAll(".streamgraph-layer").transition().duration(250).style("opacity", 1),
                    B.style("opacity", 0),
            ...
            n.on("mousemove", function(e, a) {
                ...
                mousex = mouse[0],
                mousey = mouse[1],
                s.style("top", mousey - $(".d3-tip").outerHeight() / 2 - 2 + "px"),
                mousex >= $(t).outerWidth() - $(".d3-tip").outerWidth() - r.right - 60 ? s.style("left", mousex - $(".d3-tip").outerWidth() - 30 + "px").attr("class", "d3-tip w") : s.style("left", mousex + 30 + "px").attr("class", "d3-tip e")
            })
        }),
        $(window).on("resize", a),
    ...
    function(t, e) {
        ...
        function n() {
            function t() {
                ...
                u.attr("width", s + i.left + i.right),
                p.attr("width", s + i.left + i.right),
                g.range([0, s]),
                y.range([e, 0]),
                p.select(".d3-axis-horizontal").call(v),
                p.select(".d3-axis-vertical").call(x.tickSize(0 - s)),
                p.selectAll(".d3-line").attr("d", function(t, e) {
                    return m(t.values)
                }),
            ...
            p.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")"),
            p.append("g").attr("class", "d3-axis d3-axis-vertical d3-axis-transparent");
            var b = p.selectAll(".lines").data(h);
            b.enter().append("g").attr("class", "lines").attr("id", function(t) {
                return t.name + "-line"
            }).append("path").attr("class", "d3-line d3-line-medium").style("stroke", function(t) {
                return c(t.name)
            ...
            var w = b.selectAll("circle").data(function(t) {
                return t.values
            ...
            w.style("opacity", 0).transition().duration(500).delay(500).style("opacity", 1),
            w.on("mouseover", function(t) {
                l.offset([-15, 0]).show(t),
            ...
            var k = d3.transition(b);
            k.select("path").attr("d", function(t, e) {
                return m(t.values)
            }),
            k.selectAll("circle").attr("cy", function(t, e) {
                return y(t.value)
            }).attr("cx", function(t, e) {
                return g(t.date)
            }),
            d3.transition(p).select(".d3-axis-vertical").call(x),
            d3.transition(p).select(".d3-axis-horizontal").attr("transform", "translate(0," + e + ")").call(v),
            $(window).on("resize", t),
            $(document).on("click", ".sidebar-control", t)
        }
        var r = d3.select(t)
          , i = {
            top: 5,
          ...
          , d = (d3.time.format("%b %d, '%y"),
        ["#4CAF50", "#FF5722", "#5C6BC0"])
    ...
    function() {
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function(t, e) {
            function a() {
                width = l.node().getBoundingClientRect().width - margin.left - margin.right,
                gridSize = width / new Date(e[e.length - 1].date).getHours(),
                height = (rowGap + gridSize) * d3.max(r, function(t, e) {
                    return e + 1
                }) - margin.top,
                u.attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom),
                p.attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom),
                o.range([0, width]),
                p.selectAll(".hour-group").attr("transform", function(t, e) {
                ...
                p.selectAll(".legend-group").attr("transform", "translate(" + (width / 2 - buckets * gridSize / 2) + "," + (height + margin.bottom - margin.top) + ")"),
                p.selectAll(".sales-count").attr("x", width),
                p.selectAll(".heatmap-legend-item").attr("width", gridSize).attr("x", function(t, e) {
                    return gridSize * e
                }),
            ...
            o.domain([new Date(e[0].date), d3.time.hour.offset(new Date(e[e.length - 1].date), 1)]),
            d.domain([0, d3.max(e, function(t) {
                return t.app
            })]);
            var u = l.append("svg")
            ...
            f.each(function(t) {
                h.on("mouseover", function(t, e) {
                    ...
                    d3.select(this.parentNode).select(".sales-count").text(function(t) {
                        return t.values[e].value + " sales at " + s(t.values[e].date)
                    })
                }).on("mouseout", function(t, e) {
                    d3.select(this).style("opacity", 1),
                    d3.select(this.parentNode).select(".sales-count").text(function(t, e) {
                        return d3.sum(t.values, function(t) {
                            return t.value
                        }) + " sales today"
                    })
            ...
            y.selectAll(".heatmap-legend").data([0].concat(c.quantiles()), function(t) {
                return t
            }).enter().append("g").attr("class", "heatmap-legend").append("rect").attr("class", "heatmap-legend-item").attr("x", function(t, e) {
                return gridSize * e
            }).attr("y", -8).attr("width", gridSize).attr("height", 5).style("stroke", "#fff").style("shape-rendering", "crispEdges").style("fill", function(t, e) {
                return colors[e]
            }),
            y.append("text").attr("class", "min-legend-value").attr("x", -10).attr("y", -2).style("text-anchor", "end").style("font-size", 11).style("fill", "#999").text(m),
            y.append("text").attr("class", "max-legend-value").attr("x", buckets * gridSize + 10).attr("y", -2).style("font-size", 11).style("fill", "#999").text(g),
            $(window).on("resize", a),
            $(document).on("click", ".sidebar-control", a)
        })
    }(),
    function(t, e, a) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function(t, m) {
            function g() {
                ...
                b.attr("transform", "translate(" + p(u.date) + "," + e + ")"),
                w.attr("transform", "translate(" + p(u.date) + "," + f(u.value) + ")"),
                a >= n.node().getBoundingClientRect().width - k.select("text").node().getBoundingClientRect().width - r.right - r.left ? k.select("text").attr("text-anchor", "end").attr("x", function() {
                    return p(u.date) - 15 + "px"
                }).text(o(u.date) + " - " + u.value + " sales") : k.select("text").attr("text-anchor", "start").attr("x", function() {
                    return p(u.date) + 15 + "px"
                }).text(o(u.date) + " - " + u.value + " sales")
            }
            function y() {
                i = n.node().getBoundingClientRect().width - r.left - r.right,
                d.attr("width", i + r.left + r.right),
                c.attr("width", i + r.left + r.right),
                p.range([0, i]),
                c.selectAll(".d3-axis-horizontal").call(h),
                c.selectAll(".d3-axis-subticks").attr("x1", p).attr("x2", p),
                c.selectAll(".d3-area").datum(m).attr("d", u),
                c.selectAll(".d3-crosshair-overlay").attr("width", i)
            }
            if (t)
            ...
            c.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")").call(h).selectAll(".d3-axis-subticks").data(p.ticks(d3.time.days), function(t) {
                return t
            }).enter().append("line").attr("class", "d3-axis-subticks").attr("y1", 0).attr("y2", 4).attr("x1", p).attr("x2", p),
            c.append("path").datum(m).attr("class", "d3-area").attr("d", u).style("fill", a).transition().duration(1e3).attrTween("d", function() {
                var t = d3.interpolateArray(x, m);
                return function(e) {
                    return u(t(e))
                }
            });
            var b = c.append("g").attr("class", "d3-crosshair-line").style("display", "none");
            b.append("line").attr("class", "vertical-crosshair").attr("y1", 0).attr("y2", -v).style("stroke", "#e5e5e5").style("shape-rendering", "crispEdges");
            var w = c.append("g").attr("class", "d3-crosshair-pointer").style("display", "none");
            w.append("circle").attr("r", 3).style("fill", "#fff").style("stroke", a).style("stroke-width", 1);
            var k = c.append("g").attr("class", "d3-crosshair-text").style("display", "none");
            k.append("text").attr("dy", -10).style("font-size", 12),
            c.append("rect").attr("class", "d3-crosshair-overlay").style("fill", "none").style("pointer-events", "all").attr("width", i).attr("height", e).on("mouseover", function() {
            ...
            $(window).on("resize", y),
            $(document).on("click", ".sidebar-control", y)
        })
    }("#monthly-sales-stats", 100, "#4DB6AC"),
    function(t, e, a) {
        var n = d3.select(t)
          , r = {
            top: 0,
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function(t, e) {
            function p() {
                i = n.node().getBoundingClientRect().width - r.left - r.right,
                l.attr("width", i + r.left + r.right),
                o.attr("width", i + r.left + r.right),
                c.range([0, i]),
                o.selectAll(".d3-area").datum(e).attr("d", d)
            }
            if (t)
                return console.error(t);
            ...
            $(window).on("resize", p),
            $(document).on("click", ".sidebar-control", p)
        })
    }("#messages-stats", 40, "#5C6BC0"),
    t("#new-visitors", "line", 30, 35, "basis", 750, 2e3, "#26A69A"),
    t("#new-sessions", "line", 30, 35, "basis", 750, 2e3, "#FF7043"),
    t("#total-online", "line", 30, 35, "basis", 750, 2e3, "#5C6BC0"),
    t("#server-load", "area", 30, 50, "basis", 750, 2e3, "rgba(255,255,255,0.5)"),
    function(t, e) {
        function a() {
            s = r.node().getBoundingClientRect().width - i.left - i.right,
            u.attr("width", s + i.left + i.right),
            ...
            f.range([l, s - l]),
            y.attr("width", s),
            p.selectAll(".d3-line").attr("d", m(n)),
            p.selectAll(".d3-line-circle").attr("cx", m.x()),
            p.selectAll(".d3-line-guides").attr("x1", function(t, e) {
                return f(t.date)
        ...
        v.transition().duration(1e3).delay(function(t, e) {
            return 150 * e
        ...
        x.style("opacity", 0).transition().duration(250).ease("linear").delay(1e3).style("opacity", 1),
        x.on("mouseover", function(t) {
            c.offset([-10, 0]).show(t),
            d3.select(this).transition().duration(250).attr("r", 4)
        }).on("mouseout", function(t) {
            c.hide(t),
            d3.select(this).transition().duration(250).attr("r", 3)
        ...
        d3.select(x[0][0]).on("mouseover", function(t) {
            c.offset([0, 10]).direction("e").show(t),
            d3.select(this).transition().duration(250).attr("r", 4)
        }).on("mouseout", function(t) {
            c.direction("n").hide(t),
            d3.select(this).transition().duration(250).attr("r", 3)
        }),
        d3.select(x[0][x.size() - 1]).on("mouseover", function(t) {
            c.offset([0, -10]).direction("w").show(t),
            d3.select(this).transition().duration(250).attr("r", 4)
        }).on("mouseout", function(t) {
            c.direction("n").hide(t),
            d3.select(this).transition().duration(250).attr("r", 3)
    ...
    e("#today-progress", 20, 20, "#7986CB"),
    e("#yesterday-progress", 20, 20, "#7986CB"),
    function(t, e) {
        var a = [{
          ...
          , s = d3.tip().attr("class", "d3-tip").offset([-10, 0]).direction("e").html(function(t) {
            return "<ul class='list-unstyled mb-5'><li><div class='text-size-base mb-5 mt-5'>" + t.data.icon + t.data.browser + "</div></li><li>Visits: &nbsp;<span class='text-semibold pull-right'>" + t.value + "</span></li><li>Share: &nbsp;<span class='text-semibold pull-right'>" + (100 / (i / t.value)).toFixed(2) + "%</span></li></ul>"
        })
        ...
        p.on("mouseover", function(t, e) {
            d3.select(this).transition().duration(500).ease("elastic").attr("transform", function(t) {
                return t.midAngle = (t.endAngle - t.startAngle) / 2 + t.startAngle,
                "translate(" + 2 * Math.sin(t.midAngle) + "," + 2 * -Math.cos(t.midAngle) + ")"
            })
    ...
    a("#hours-available-bars", 24, 40, !0, "elastic", 1200, 50, "#EC407A", "hours"),
    a("#goal-bars", 24, 40, !0, "elastic", 1200, 50, "#5C6BC0", "goal"),
    a("#members-online", 24, 50, !0, "elastic", 1200, 50, "rgba(255,255,255,0.5)", "members"),
    n("#hours-available-progress", 38, 2, "#F06292", .68, "icon-watch text-pink-400", "Hours available", "64% average"),
    n("#goal-progress", 38, 2, "#5C6BC0", .82, "icon-trophy3 text-indigo-400", "Productivity goal", "87% average"),
    function(t, e) {
        function a(t) {
            return t.randomizer || (t.randomizer = n(t)),
            t.ranges = t.ranges.map(t.randomizer),
            t.markers = t.markers.map(t.randomizer),
            t.measures = t.measures.map(t.randomizer),
            t
        }
        function n(t) {
        ...
        !function() {
            ...
            d3.bullet = function() {
                function l(t) {
                    t.each(function(t, e) {
                        function a() {
                            s = d3.select("#bullets").node().getBoundingClientRect().width - i.left - i.right,
                            w = r(v),
                            v.range(d ? [s, 0] : [0, s]),
                            y.selectAll(".bullet-measure").attr("width", w).attr("x", d ? v : 0),
                            y.selectAll(".bullet-range").attr("width", w).attr("x", d ? v : 0),
                            y.selectAll(".bullet-marker").attr("x1", v).attr("x2", v),
                            y.selectAll(".bullet-tick").attr("transform", n(v))
                        }
                        var l = u.call(this, t, e).slice().sort(d3.descending)
                        ...
                        }).attr("width", b).attr("height", h).attr("rx", 2).attr("x", d ? x : 0).transition().duration(c).attr("width", w).attr("x", d ? v : 0),
                        k.transition().duration(c).attr("x", d ? v : 0).attr("width", w).attr("height", h);
                        ...
                        }).attr("width", b).attr("height", h / 5).attr("x", d ? x : 0).attr("y", h / 2.5).style("shape-rendering", "crispEdges"),
                        A.transition().duration(c).attr("width", w).attr("x", d ? v : 0),
                        A.transition().duration(c).attr("width", w).attr("height", h / 5).attr("x", d ? v : 0).attr("y", h / 2.5);
                        var z = y.selectAll(".bullet-marker").data(o);
                        z.enter().append("line").attr("class", function(t, e) {
                            return "bullet-marker bullet-marker-" + (e + 1)
                        }).attr("x1", x).attr("x2", x).attr("y1", h / 6).attr("y2", 5 * h / 6),
                        z.transition().duration(c).attr("x1", v).attr("x2", v),
                        z.transition().duration(c).attr("x1", v).attr("x2", v).attr("y1", h / 6).attr("y2", 5 * h / 6);
                        ...
                        B.append("line").attr("y1", h).attr("y2", 7 * h / 6 + 3),
                        B.append("text").attr("text-anchor", "middle").attr("dy", "1em").attr("y", 7 * h / 6 + 4).text(C),
                        B.transition().duration(c).attr("transform", n(v)).style("opacity", 1);
                        var F = M.transition().duration(c).attr("transform", n(v)).style("opacity", 1);
                        F.select("line").attr("y1", h + 3).attr("y2", 7 * h / 6 + 3),
                        F.select("text").attr("y", 7 * h / 6 + 4),
                        M.exit().transition().duration(c).attr("transform", n(v)).style("opacity", 1e-6).remove(),
                        $(window).on("resize", a),
                        $(document).on("click", ".sidebar-control", a)
                    }),
                    d3.timer.flush()
                ...
                ,
                l.markers = function(t) {
                    return arguments.length ? (p = t,
                    l) : p
                }
                ,
                l.measures = function(t) {
        ...
        d3.json("assets/demo_data/dashboard/bullets.json", function(t, n) {
            function o() {
                s = r.node().getBoundingClientRect().width - i.left - i.right,
                d.attr("width", s + i.left + i.right),
                c.attr("width", s + i.left + i.right),
                c.selectAll(".bullet-subtitle").attr("x", s)
            }
            if (t)
                return console.error(t);
            var d = r.selectAll("svg").data(n).enter().append("svg")
              , c = d.attr("class", function(t, e) {
                return "bullet-" + (e + 1)
            }).attr("width", s + i.left + i.right).attr("height", e + i.top + i.bottom).append("g").attr("transform", "translate(" + i.left + "," + i.top + ")").call(l)
              , u = c.append("g").style("text-anchor", "start");
            u.append("text").attr("class", "bullet-title").attr("y", -10).text(function(t) {
                return t.title
            }),
            u.append("text").attr("class", "bullet-subtitle").attr("x", s).attr("y", -10).style("text-anchor", "end").text(function(t) {
                return t.subtitle
            }).style("opacity", 0).transition().duration(500).delay(500).style("opacity", 1);
            var p = function() {
                c.datum(a).call(l.duration(750))
            }
              , f = [];
            ...
            ,
            $(window).on("resize", o),
            $(document).on("click", ".sidebar-control", o)
        })
    }("#bullets", 80),
